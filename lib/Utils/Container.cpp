...
MemRef<T, N>::MemRef(const T *data, intptr_t sizes[N], intptr_t strides[N],
                     intptr_t offset) {
...
template <typename T, std::size_t N>
MemRef<T, N>::MemRef(intptr_t sizes[N], intptr_t strides[N], T init) {
  static_assert(N >= 1 && N <= 4, "MemRef size not supported.");
...
MemRef<T, N>::MemRef(intptr_t results, intptr_t sizes[N], intptr_t strides[N],
                     intptr_t offset) {
...
template <typename T, size_t N>
MemRef<T, N>::MemRef(cv::Mat image, intptr_t sizes[N], intptr_t strides[N]) {
...
template <typename T, std::size_t N>
MemRef<T, N> MemRef<T, N>::transpose(const std::vector<size_t> &dims) {
  static_assert(N == 2 || N == 3, "Currently only support 2D and 3D memref.");

...
def get_info_and_samples(file: str) -> Tuple[Dict[str, int], np.ndarray]:
    ...
    :return: A dictionary that contains basic information and a numpy array that contains the sample data.
    """
...
def get_frequency_domain(file: str):
    ...
    :param file: The path of the .wav file.
    :return: The power data after FFT on sample data array (y-axis) and the frequency array (x-axis).
    """
    # get basic information and samples data
    info, samples = get_info_and_samples(file)
    framerate = info['framerate']
    ...
    fft_complex = fft(samples)
    power = np.abs(fft_complex)

    # get frequency array
    freq = fftfreq(len(samples), 1 / framerate)

    return power, freq


def compare_wave(file1: str, file2: str, part: str = "body", granularity: Union[int, float] = 5000, save: bool = True):
    """
    Plot the comparison figures between before and after the process in time-domain and frequency-domain.
    :param file1: The path of the .wav file before process.
    :param file2: The path of the .wav file after process.
    :param part: Choose a part of the time-domain figure to show. Possible values are "head", "body", "tail" and "all".
    :param granularity: Choose how many points is to be plottes. It should be either a portion(float between 0~1) or a
                        number(int).
    :param save: A bool value to decide whether or not to save the figure.
    :return: None.
    """
    # plot time-domain figure
    before, time1 = get_time_domain(file1)
    after, time2 = get_time_domain(file2)
    # As we aim to compare signals between before and after the process, we have to promise that the time slot arrays
    # remain the same.
    assert (time1 == time2).all(), "Time slot arrays are not equal!"

    # As the time-domain figure is usually too large to see the difference, we provide customized options to let the
    # user decide which part should be plotted.
    # calculate the number of points to be plotted
    N = len(time1)
    if 0 <= granularity <= 1:                # the granularity could be portion
        n = int(N * granularity)
        assert n > 0, "Few points to plot!"
    elif granularity % 1 == 0:               # the granularity could be number of points to be plotted
        n = int(granularity)
    else:
        raise ValueError("Parameter 'granularity' should be either a portion(float between 0~1) or a number(int)!")
    ...
    # plot time-domain figure
    # We reserve the whole plot to show the difference of the high-frequency part.
    ...
    if save:
        plt.savefig("compare_res.png", dpi=200)
    plt.show()

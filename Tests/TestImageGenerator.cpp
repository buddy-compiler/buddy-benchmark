                           ...
                           cv::Point anchor, double delta, boundaryOption option, std::string name)
{
  if (option == boundaryOption::BORDER_CONSTANT)
    ...
    cv::filter2D(img, expectedImg, ddepth, kernel, anchor, delta, cv::BORDER_REPLICATE);
  else if (option == boundaryOption::BORDER_REFLECT)
    cv::filter2D(img, expectedImg, ddepth, kernel, anchor, delta, cv::BORDER_REFLECT);
  else if (option == boundaryOption::BORDER_REFLECT101)
    cv::filter2D(img, expectedImg, ddepth, kernel, anchor, delta, cv::BORDER_REFLECT101);
  ...
  for (std::ptrdiff_t i = 0; i < kernel.rows; ++i)
  {
    for (std::ptrdiff_t j = 0; j < kernel.cols; ++j)
    {
      ...
      generateExpectedImage(origImg, expImg, ddepth, kernel, anchor, delta, 
                            boundaryOption::BORDER_CONSTANT, partialName + "BORDER_CONSTANT.png");

      generateExpectedImage(origImg, expImg, ddepth, kernel, anchor, delta, 
                            boundaryOption::BORDER_REPLICATE, partialName + "BORDER_RELPICATE.png");

      generateExpectedImage(origImg, expImg, ddepth, kernel, anchor, delta, 
                            boundaryOption::BORDER_REFLECT, partialName + "BORDER_REFLECT.png");

      generateExpectedImage(origImg, expImg, ddepth, kernel, anchor, delta, 
                            boundaryOption::BORDER_REFLECT101, partialName + "BORDER_REFLECT101.png");
    }
  }
}

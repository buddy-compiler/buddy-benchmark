  ...
  affine.for %b_row_idx = 0 to %b_row {
    affine.for %a_row_idx = 0 to %a_row {
      affine.for %b_col_idx = 0 to #map(%b_col) {
        ...
        scf.if %tail_flag {
          %b_vec = affine.vector_load %b[%b_row_idx, %b_col_idx * 16] : memref<?x?xf32>, vector<16xf32>
          %c_vec = affine.vector_load %c[%a_row_idx, %b_col_idx * 16] : memref<?x?xf32>, vector<16xf32>
          %result_vec = vector.fma %a_vec, %b_vec, %c_vec : vector<16xf32>
          affine.vector_store %result_vec, %c[%a_row_idx, %b_col_idx * 16] : memref<?x?xf32>, vector<16xf32>
        } else {
          %mask_vec = vector.create_mask %tail_len : vector<16xi1>
          %b_col_idx_tail = arith.muli %b_col_idx, %step : index
          %b_vec_tail = vector.maskedload %b[%b_row_idx, %b_col_idx_tail], %mask_vec, %c0_f32_vec : memref<?x?xf32>, vector<16xi1>, vector<16xf32> into vector<16xf32>
          %c_vec_tail = vector.maskedload %c[%a_row_idx, %b_col_idx_tail], %mask_vec, %c0_f32_vec : memref<?x?xf32>, vector<16xi1>, vector<16xf32> into vector<16xf32>
          %result_vec_tail = vector.fma %a_vec, %b_vec_tail, %c_vec_tail : vector<16xf32>
          vector.maskedstore %c[%a_row_idx, %b_col_idx_tail], %mask_vec, %result_vec_tail : memref<?x?xf32>, vector<16xi1>, vector<16xf32>
        }
      }
    }
  }

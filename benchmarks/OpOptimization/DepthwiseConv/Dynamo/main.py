...
# ===---------------------------------------------------------------------------
#
# This file implements the dynamo entry for benchmark DepthwiseConv on GPU.
...
def main():
    ...
    s_1 = depthwise_conv_torch(data, out, weight, k, p, s)
    evaluate_operation(
        s_1, inputs=data, optimization="torch_depthwiseconv_default", log=log
    )

    s_2 = depthwise_conv_compiled(data, out, weight, k, p, s)
    evaluate_operation(
        s_2, inputs=data, optimization="torch_depthwiseconv_dynamo", log=log
    )

...
# ===---------------------------------------------------------------------------
#
# This file implements the TVM optimization entry for Conv on GPU.
...
from gpu_conv_manual import *
from gpu_conv_auto import *
...
def main():
    ...
    sch, arg_bufs = gpu_conv_default(oc, ic, n, k, p, s)
    evaluate_operation(
        sch,
        arg_bufs,
        target=target,
        inputs=(data_x, data_k, data_y),
        optimization="gpu_conv_default",
        log=log,
    )

    sch, arg_bufs = gpu_conv_tiling(oc, ic, n, k, p, s)
    evaluate_operation(
        sch,
        arg_bufs,
        target=target,
        inputs=(data_x, data_k, data_y),
        optimization="gpu_conv_tiling",
        log=log,
    )

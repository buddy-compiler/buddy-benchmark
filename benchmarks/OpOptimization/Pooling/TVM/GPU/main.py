...
# ===---------------------------------------------------------------------------
#
# This file implements the TVM optimization entry for Pooling on GPU.
...
def report_performance(log):
    ...
    print(split_line)
    for result in log:
        formatted_time = "{:.5f}".format(result[1])
        formatted_performance = "{:.5f}".format(baseline / result[1])
        print(
...
def main():
    ...
    sch, arg_bufs = schedule_max(size)
    evaluate_operation(
        sch,
        arg_bufs,
        target=target,
        inputs=(data, _, out_max),
        optimization="schedule_max",
        log=log,
    )

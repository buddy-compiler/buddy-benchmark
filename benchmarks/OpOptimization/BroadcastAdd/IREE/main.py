...
# ===---------------------------------------------------------------------------
#
# This file implements the IREE entry for benchmark broadcastAdd on GPU.
...
def main():
    ...
    b_dim1 = a_dim2
    b_dim2 = 2048
    model, b_tensor = torch_BroadcastAdd(b_dim1, b_dim2)
    ...
    invoker = iree_BroadcastAdd(model, example_input)
    evaluate_operation(invoker, inputs=example_input, optimization="IREE", log=log)
    s = model
    evaluate_operation(s, inputs=example_input, optimization="torch_cpu", log=log)

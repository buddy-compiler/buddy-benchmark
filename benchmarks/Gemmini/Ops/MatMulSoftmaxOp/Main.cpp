...
using namespace buddy::benchmark::gemmini;
...
extern "C" {
void _mlir_ciface_gemmini_matmul_softmax(MemRef<int8_t, 2> *input0,
...
void nativeMatmul(int8_t *inputA, int8_t *inputB, int8_t *outputC,
  ...
  tiled_matmul_auto(_SIZE_M, _SIZE_N, _SIZE_K, inputA, inputB, inputBias,
                    ...
                    MVIN_SCALE_IDENTITY, SOFTMAX, ACC_SCALE_IDENTITY, BERT_SCALE,
...
int main() {
  ...
  buddyMatmul(outputExpected, _mlir_ciface_gemmini_matmul_softmax,
              "Buddy Gemmini MatMulSoftmax");

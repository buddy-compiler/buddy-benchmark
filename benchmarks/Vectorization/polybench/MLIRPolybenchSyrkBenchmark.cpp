...
static auto initializeMemRefs(const std::vector<size_t> &size) {
  ...
  _mlir_ciface_syrk_init_array(n, m, &alpha, &beta, &C, &A);

  return std::make_tuple(n, m, std::move(alpha), std::move(beta), std::move(C),
                         std::move(A));
}

// Runs the provided kernel for the syrk benchmark.
static void MLIRPolybenchSyrk(benchmark::State &state, KernelFunc kernel) {
  ...
  for (auto _ : state) {
    ...
    auto [n, m, alpha, beta, C, A] = initializeMemRefs(size);
    state.ResumeTiming();
    kernel(n, m, alpha.getData()[0], beta.getData()[0], &C, &A);
  }
}

// Run the kernel and return the memref instance for verification.
static MemRef<double, 2> runMLIRPolybenchSyrk(KernelFunc kernel,
                                              size_t size_id) {
...
extern "C" {
void _mlir_ciface_syrk_kernel_scalar(int, int, double, double,
                                     MemRef<double, 2> *, MemRef<double, 2> *);

void _mlir_ciface_syrk_kernel_autovec(int, int, double, double,
                                      MemRef<double, 2> *, MemRef<double, 2> *);
/// [Step 1] Add function of new methods here.
}

BENCHMARK_CAPTURE(MLIRPolybenchSyrk, scalar, _mlir_ciface_syrk_kernel_scalar)
    ->DenseRange(0, DATASET_SIZES.size() - 1)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(MLIRPolybenchSyrk, autovec, _mlir_ciface_syrk_kernel_autovec)
    ->DenseRange(0, DATASET_SIZES.size() - 1)
    ->Unit(benchmark::kMillisecond);
/// [Step 2] Register new benchmarks here.

void verifyResultMLIRPolybenchSyrk(size_t size_id) {
  const std::string benchmarkName =
      "syrk-" + polybench::getPolybenchDatasetSizeName(size_id);

  auto refC = runMLIRPolybenchSyrk(_mlir_ciface_syrk_kernel_scalar, size_id);

  auto vecC = runMLIRPolybenchSyrk(_mlir_ciface_syrk_kernel_autovec, size_id);
...
void generateResultMLIRPolybenchSyrk(size_t size_id) {
  const std::string benchmarkName =
      "syrk-" + polybench::getPolybenchDatasetSizeName(size_id);
  auto C = runMLIRPolybenchSyrk(_mlir_ciface_syrk_kernel_scalar, size_id);
  std::cout << "------------------------------------------------" << std::endl;

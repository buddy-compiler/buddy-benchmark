//===- MLIRLinpackCDaxpyUnrollF32.mlir ------------------------------------===//
//
...
func.func @mlir_linpackcdaxpyunrollf32(%n : i32, %da : f32, %dx: memref<?xf32>, %incx : i32,
  ...
  ^continue4:
    scf.for %i_2 = %m to %n_index step %c4 {
      ...
      %new_dy_val_2 = arith.addf %dy_val_2, %result_2 : f32
      memref.store %new_dy_val_2, %dy[%i_2] : memref<?xf32>

      %i_2_1 = arith.addi %i_2, %c1 : index
      %dx_val_3 = memref.load %dx[%i_2_1] : memref<?xf32>
      %dy_val_3 = memref.load %dy[%i_2_1] : memref<?xf32>
      %result_3 = arith.mulf %da, %dx_val_3 : f32
      %new_dy_val_3 = arith.addf %dy_val_3, %result_3 : f32
      memref.store %new_dy_val_3, %dy[%i_2_1] : memref<?xf32>

      %i_2_2 = arith.addi %i_2, %c2 : index
      %dx_val_4 = memref.load %dx[%i_2_2] : memref<?xf32>
      %dy_val_4 = memref.load %dy[%i_2_2] : memref<?xf32>
      %result_4 = arith.mulf %da, %dx_val_4 : f32
      %new_dy_val_4 = arith.addf %dy_val_4, %result_4 : f32
      memref.store %new_dy_val_4, %dy[%i_2_2] : memref<?xf32>

      %i_2_3 = arith.addi %i_2, %c3 : index
      %dx_val_5 = memref.load %dx[%i_2_3] : memref<?xf32>
      %dy_val_5 = memref.load %dy[%i_2_3] : memref<?xf32>
      %result_5 = arith.mulf %da, %dx_val_5 : f32
      %new_dy_val_5 = arith.addf %dy_val_5, %result_5 : f32
      memref.store %new_dy_val_5, %dy[%i_2_3] : memref<?xf32>
    }
    return

...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for OpenCV Dilate2D.
//
//===----------------------------------------------------------------------===//
...
Mat inputImageDilate2D, kernelDilate2D, outputDilate2D;
...
void initializeOpenCVDilate2D(char **argv) {
  inputImageDilate2D = imread(argv[1], IMREAD_GRAYSCALE);

  kernelDilate2D = Mat(get<1>(kernelMap1[argv[4]]), get<2>(kernelMap1[argv[4]]),
...
static void OpenCV_Dilate2D_Constant_Padding(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
        dilate(inputImageDilate2D, outputDilate2D, kernelDilate2D, cv::Point(1, 1), 5,

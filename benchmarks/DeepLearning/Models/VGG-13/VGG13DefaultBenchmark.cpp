...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for e2e VGG13.
// The vgg13_tosa.mlir is generated from torch-mlir project.
//
//===----------------------------------------------------------------------===//
...
// Declare the vgg13 C interface.
extern "C" {
void _mlir_ciface_vgg13(MemRef<float, 2> *output, Img<float, 4> *input);
}

const cv::Mat imagePreprocessing() {
  cv::Mat inputImage =
      cv::imread("../../benchmarks/DeepLearning/Models/VGG-13/Images/"
...
void BM_vgg13(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      _mlir_ciface_vgg13(&output, &input);
    }
  }
}
...
std::string getLabel(int idx) {
  std::ifstream in("../../benchmarks/DeepLearning/Models/VGG-13/Labels.txt");
...
BENCHMARK(BM_vgg13)->Arg(1)->Unit(benchmark::kMillisecond);

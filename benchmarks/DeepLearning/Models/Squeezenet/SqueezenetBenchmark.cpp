...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for e2e Squeeznenet.
//
//===----------------------------------------------------------------------===//
...
// Declare the Squeezenet C interface.
extern "C" {
void _mlir_ciface_squeezenet(MemRef<float, 2> *output, MemRef<float, 4> *input);
}

const cv::Mat imagePreprocessing() {

  cv::Mat inputImage = cv::imread(
      "../../benchmarks/DeepLearning/Models/Squeezenet/Images/ice-cream.png");
...
void BM_Squeezenet(benchmark::State &state) {
  for (auto _ : state) {
    for (int i = 0; i < state.range(0); ++i) {
      _mlir_ciface_squeezenet(&output, &input);
    }
  }
}
...
std::string getLabel(int idx) {
  std::ifstream in(
      "../../benchmarks/DeepLearning/Models/Squeezenet/Labels.txt");

...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for divf operation.
//
//===----------------------------------------------------------------------===//
...
namespace {
// Declare the math.divf C interface.
extern "C" {
void _mlir_ciface_divf_scalar(MemRef<float, 4> *input1, MemRef<float, 4> *input2, MemRef<float, 4> *output);
void _mlir_ciface_divf_auto_vectorization(MemRef<float, 4> *input1, MemRef<float, 4> *input2, MemRef<float, 4> *output);
}

#define DEFINE_DIVF_BENCHMARK(name, func)                                     \
  void BM_DIVF_##name(benchmark::State &state) {                              \
...
DEFINE_DIVF_BENCHMARK(SCALAR, _mlir_ciface_divf_scalar)
DEFINE_DIVF_BENCHMARK(AutoVectorization, _mlir_ciface_divf_auto_vectorization)
} // namespace

// Register benchmark cases.
BENCHMARK(BM_DIVF_SCALAR)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_DIVF_AutoVectorization)->Unit(benchmark::kMillisecond);
...
void verification() {
  ...
  // Perform all the divf implementations.
  _mlir_ciface_divf_scalar(&inputMemRef1, &inputMemRef2, &outputScalar);
  _mlir_ciface_divf_auto_vectorization(&inputMemRef1, &inputMemRef2, &outputAutoVectorization);

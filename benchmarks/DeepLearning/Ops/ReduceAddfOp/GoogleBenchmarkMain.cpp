...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for reduce addf operation.
//
//===----------------------------------------------------------------------===//
...
namespace {
// Declare the math.addf C interface.
extern "C" {
void _mlir_ciface_addf_scalar(MemRef<float, 4> *input1, MemRef<float, 4> *input2, MemRef<float, 4> *output);
void _mlir_ciface_addf_auto_vectorization(MemRef<float, 4> *input1, MemRef<float, 4> *input2, MemRef<float, 4> *output);
}

#define DEFINE_ADDF_BENCHMARK(name, func)                                     \
  void BM_ADDF_##name(benchmark::State &state) {                              \
...
DEFINE_ADDF_BENCHMARK(SCALAR, _mlir_ciface_addf_scalar)
DEFINE_ADDF_BENCHMARK(AutoVectorization, _mlir_ciface_addf_auto_vectorization)
} // namespace

// Register benchmark cases.
BENCHMARK(BM_ADDF_SCALAR)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_ADDF_AutoVectorization)->Unit(benchmark::kMillisecond);
...
void verification() {
  ...
  _mlir_ciface_addf_scalar(&inputMemRef1, &inputMemRef2, &outputScalar);
  _mlir_ciface_addf_auto_vectorization(&inputMemRef1, &inputMemRef2, &outputAutoVectorization);

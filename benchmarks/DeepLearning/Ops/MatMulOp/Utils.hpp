...
//===----------------------------------------------------------------------===//
//
// This file implements MatMulOp helper functions.
//
//===----------------------------------------------------------------------===//
...
void verify(const std::string &name, DATA_TYPE *A, DATA_TYPE *B, int rows, int cols) {
  ...
  for (int i = 0; i < rows; i++) {
    ...
    if (std::fabs(A[k] - B[k]) > epsilon) {
      ...
      std::cout << "i=" << i << " j=" << j << ":\tA[i][j]=" << A[k] << " B[i][j]=" << B[k]
                << std::endl;
      isPass = false;
      break;
    }

...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for negf operation.
//
//===----------------------------------------------------------------------===//
...
namespace {
// Declare the math.negf C interface.
extern "C" {
void _mlir_ciface_negf_scalar(MemRef<float, 4> *input, MemRef<float, 4> *output);
void _mlir_ciface_negf_auto_vectorization(MemRef<float, 4> *input, MemRef<float, 4> *output);
}

#define DEFINE_NEGF_BENCHMARK(name, func)                                     \
  void BM_NEGF_##name(benchmark::State &state) {                              \
...
DEFINE_NEGF_BENCHMARK(SCALAR, _mlir_ciface_negf_scalar)
DEFINE_NEGF_BENCHMARK(AutoVectorization, _mlir_ciface_negf_auto_vectorization)
} // namespace

// Register benchmark cases.
BENCHMARK(BM_NEGF_SCALAR)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_NEGF_AutoVectorization)->Unit(benchmark::kMillisecond);
...
void verification() {
  ...
  // Perform all the negf implementations.
  _mlir_ciface_negf_scalar(&inputMemRef, &outputScalar);
  _mlir_ciface_negf_auto_vectorization(&inputMemRef, &outputAutoVectorization);

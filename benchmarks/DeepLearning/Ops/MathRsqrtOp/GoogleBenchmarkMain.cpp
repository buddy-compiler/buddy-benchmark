...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for rsqrt operation.
//
//===----------------------------------------------------------------------===//
...
namespace {
// Declare the math.rsqrt C interface.
extern "C" {
void _mlir_ciface_rsqrt_scalar(MemRef<float, 4> *input, MemRef<float, 4> *output);
void _mlir_ciface_rsqrt_auto_vectorization(MemRef<float, 4> *input, MemRef<float, 4> *output);
}

#define DEFINE_RSQRT_BENCHMARK(name, func)                                    \
  void BM_RSQRT_##name(benchmark::State &state) {                             \
...
DEFINE_RSQRT_BENCHMARK(SCALAR, _mlir_ciface_rsqrt_scalar)
DEFINE_RSQRT_BENCHMARK(AutoVectorization, _mlir_ciface_rsqrt_auto_vectorization)
} // namespace

// Register benchmark cases.
BENCHMARK(BM_RSQRT_SCALAR)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_RSQRT_AutoVectorization)->Unit(benchmark::kMillisecond);
...
void verification() {
  ...
  // Perform all the rsqrt implementations.
  _mlir_ciface_rsqrt_scalar(&inputMemRef, &outputScalar);
  _mlir_ciface_rsqrt_auto_vectorization(&inputMemRef, &outputAutoVectorization);

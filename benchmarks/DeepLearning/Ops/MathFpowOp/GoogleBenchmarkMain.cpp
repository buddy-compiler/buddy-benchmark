...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for pow operation.
//
//===----------------------------------------------------------------------===//
...
namespace {
// Declare the math.powf C interface.
extern "C" {
void _mlir_ciface_fpow_scalar(MemRef<float, 4> *input, MemRef<float, 4> *output);
void _mlir_ciface_fpow_auto_vectorization(MemRef<float, 4> *input, MemRef<float, 4> *output);
}

#define DEFINE_FPOW_BENCHMARK(name, func)                                     \
  void BM_FPOW_##name(benchmark::State &state) {                              \
...
DEFINE_FPOW_BENCHMARK(SCALAR, _mlir_ciface_fpow_scalar)
DEFINE_FPOW_BENCHMARK(AutoVectorization, _mlir_ciface_fpow_auto_vectorization)
} // namespace

// Register benchmark cases.
BENCHMARK(BM_FPOW_SCALAR)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_FPOW_AutoVectorization)->Unit(benchmark::kMillisecond);
...
void verification() {
  ...
  // Perform all the powf implementation.
  _mlir_ciface_fpow_scalar(&inputMemRef, &outputScalar);
  _mlir_ciface_fpow_auto_vectorization(&inputMemRef, &outputAutoVectorization);

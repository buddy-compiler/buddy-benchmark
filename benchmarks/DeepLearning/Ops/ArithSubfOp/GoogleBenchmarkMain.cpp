...
//===----------------------------------------------------------------------===//
//
// This file implements the benchmark for subf operation.
//
//===----------------------------------------------------------------------===//
...
namespace {
// Declare the math.subf C interface.
extern "C" {
void _mlir_ciface_subf_scalar(MemRef<float, 4> *input1, MemRef<float, 4> *input2, MemRef<float, 4> *output);
void _mlir_ciface_subf_auto_vectorization(MemRef<float, 4> *input1, MemRef<float, 4> *input2, MemRef<float, 4> *output);
}

#define DEFINE_SUBF_BENCHMARK(name, func)                                     \
  void BM_SUBF_##name(benchmark::State &state) {                              \
...
DEFINE_SUBF_BENCHMARK(SCALAR, _mlir_ciface_subf_scalar)
DEFINE_SUBF_BENCHMARK(AutoVectorization, _mlir_ciface_subf_auto_vectorization)
} // namespace

// Register benchmark cases.
BENCHMARK(BM_SUBF_SCALAR)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_SUBF_AutoVectorization)->Unit(benchmark::kMillisecond);
...
void verification() {
  ...
  // Perform all the subf implementations.
  _mlir_ciface_subf_scalar(&inputMemRef1, &inputMemRef2, &outputScalar);
  _mlir_ciface_subf_auto_vectorization(&inputMemRef1, &inputMemRef2, &outputAutoVectorization);

...
void MLIRVerification(float *outputExpected, MLIRFunctionType MLIRFunc,
  ...
  batch_matmul::verify<float>(outputExpected, outputOptimized, BATCH_SIZE,
...
void _mlir_ciface_batch_matmul_vectorization(MemRef<float, 3> *A,
                                             MemRef<float, 3> *B,
                                             MemRef<float, 3> *C);
void _mlir_ciface_batch_matmul_tile(MemRef<float, 3> *A, MemRef<float, 3> *B,
                                    MemRef<float, 3> *C);
...
BENCHMARK_CAPTURE(DL_OPS_BATCH_MATMUL, Vectorization,
    ...
    ->Unit(benchmark::kMillisecond)
    ->Iterations(_NUM_ITER);
BENCHMARK_CAPTURE(DL_OPS_BATCH_MATMUL, Tile, _mlir_ciface_batch_matmul_tile)
    ->Unit(benchmark::kMillisecond)
    ->Iterations(_NUM_ITER);
...
int main(int argc, char **argv) {
  ...
  input1 = batch_matmul::allocArray<float>(BATCH_SIZE * _SIZE_M, _SIZE_K);
  input2 = batch_matmul::allocArray<float>(BATCH_SIZE * _SIZE_K, _SIZE_N);

#    iir.py - IIR filter class for audio validation
...
# This File is for testing of the IIR filter in buddy-mlir.

from cffi import FFI
import matplotlib.pyplot as plt
from scipy.signal import butter, sosfilt, sosfreqz, lfilter, iirfilter
import scipy as sp
import numpy as np
import os
import sys
...
class IIRTest(AudioTest):
    ...
    def __init__(self, test_name, test_type, test_params=default_param):
        super(IIRTest, self).__init__(test_name, test_type, test_params)
    ...
    def run_file_test(self):
        ...
        Wn = 1000.0 / (48000 / 2)  # Normalized cutoff frequency (1 is Nyquist)
        b, a = iirfilter(N, Wn, btype='low', ftype='butter')
        ...
        kernel = ffi.cast("float *", b.ctypes.data)
        output = ffi.new("float[]", sp_nasa.size)
        print(f"input size: {sp_nasa.size}")
        # out = C.iir(input, kernel, output,
        #             sp_nasa.size, b[0].size, sp_nasa.size)
        out = C.iirFilt(input,output, sp_nasa.size, sp_nasa.size)
        print("iir finished")
        out = ffi.unpack(out, sp_nasa.size)

        # scipy iir filtering
        out_sp = lfilter(b, a, sp_nasa)

        pwd = os.getcwd()
        print(f"Writing files to {pwd}/iir_scipy_buddy.txt")

        file = open("iir_scipy_buddy.txt", "w")
